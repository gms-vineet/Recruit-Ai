...
//       // If tokens are present here, set session now
//       if (h.access_token && h.refresh_token) {
...
//         });
//         if (error) {
//           setLinkError(error.message || "session_error");
//           setChecking(false);
//           return;
//         }
//       }
...
//       const { data } = await supabase.auth.getSession();
//       if (!data.session) {
//         // Stay on this page and show a helpful message instead of redirecting to "/"
//         setLinkError("no_session");
//       }
//       setChecking(false);
//     };
//     init();
...
//               <div className="mb-6 rounded-md border border-red-500/40 bg-red-500/10 text-red-300 px-4 py-3 text-sm">
//                 {linkError === "otp_expired"
//                   ? "This link is invalid or has expired. Please request a new invite/reset."
//                   : linkError === "no_session"
//                   ? <>We couldn’t find a valid session. Please <b>open the invite/reset link from your email again</b>.</>
//                   : `Error: ${linkError}`}
//                 <div className="mt-3">
//                   <Link to="/" className="underline">Back to login</Link>
//                 </div>
//               </div>
//             )}
...
export default function SbSetPassword() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // if tokens are present (from /auth/callback), set session now
      if (h.access_token && h.refresh_token) {
        const { error } = await supabase.auth.setSession({
          access_token: h.access_token,
          refresh_token: h.refresh_token,
      ...
      const { data } = await supabase.auth.getSession();
      if (!data.session) setLinkError("no_session");
  ...
  const formik = useFormik({
    ...
    onSubmit: async ({ password }, helpers) => {
      ...
      await supabase.auth.signOut();              // we only used Supabase to set password
      navigate(`/?${mode === "reset" ? "reset" : "setup"}=done`, { replace: true }); // go to login with banner
    },
  });
        ...
        <main className="flex-1 grid place-items-center px-4">
          <div className="w-full max-w-md">
            <div className="text-center mb-8">
              ...
              <p className="mt-2 text-slate-600 dark:text-slate-200">Use the password you will sign in with.</p>
            </div>
            ...
            {!linkError && (
              <form onSubmit={formik.handleSubmit} className="space-y-6" noValidate>
                <div className="flex flex-col">
                  <label htmlFor="password" className="mb-2 font-medium text-slate-700 dark:text-slate-300">New password</label>
                  <input
                    id="password"
                    name="password"
                    type="password"
                    ...
                    aria-invalid={!!passwordError}
                  />
                  {passwordError && <p className="mt-2 text-sm text-red-500">{formik.errors.password}</p>}
                </div>

                {formik.status && (
                  <div className="text-sm text-red-400 border border-red-400/30 rounded-md p-3">{formik.status}</div>
                )}
